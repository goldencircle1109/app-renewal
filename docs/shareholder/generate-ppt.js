...
slide = pres.addSlide();
slide.background = { color: WHITE };
slide.addText("AI 개발팀 구축 효과", { x: 0.5, y: 0.3, w: 12, fontSize: 28, color: NAVY, bold: true, fontFace: "Arial Black" });
slide.addText("6개 AI 팀 + 16개 QA 에이전트 = 3~4명 개발팀 대체, 인건비 연 3.6억원 절감", {
  x: 0.5, y: 0.9, w: 12, fontSize: 13, color: NAVY, fontFace: "Arial", fill: { color: ICE }, margin: [8, 12, 8, 12]
});
...
const teamData = [
  ["T1 일정기획", "2개", "PM 대체"],
  ["T2 구현설계", "6개", "시니어 개발자 대체"],
  ["T3 개발", "3~30개", "개발자 3명 대체"],
  ["T4 QA", "16개", "QA 전문가 대체"],
  ["T5 배포", "2개", "DevOps 대체"],
  ["T6 보안", "2개", "보안 전문가 대체"],
];
slide.addTable(
  [["AI 팀", "에이전트", "대체 효과"].map(t => ({ text: t, options: { fontSize: 11, bold: true, color: WHITE, fill: { color: NAVY } } })),
   ...teamData.map(r => r.map(t => ({ text: t, options: { fontSize: 11, color: DARK, fill: { color: LIGHT_BG } } })))],
  { x: 0.5, y: 1.6, w: 6, colW: [2, 1.5, 2.5], border: { color: "DDDDDD", pt: 0.5 }, margin: [4, 8, 4, 8] }
);

// Cost comparison
slide.addText("인건비 절감", { x: 7, y: 1.4, w: 5.5, fontSize: 16, color: NAVY, bold: true, fontFace: "Arial" });
const costData = [
  ["일반 스타트업", "월 3,200만원"],
  ["AI 개발팀", "월 ~20만원"],
  ["절감율", "99.4%"],
...
slide.addTable(
  [["항목", "금액"].map(t => ({ text: t, options: { fontSize: 11, bold: true, color: WHITE, fill: { color: "2ECC71" } } })),
   ...costData.map(r => r.map((t, i) => ({ text: t, options: { fontSize: 11, color: i === 1 && t.includes("절감") ? "2ECC71" : DARK, bold: i === 1, fill: { color: LIGHT_BG } } })))],
  { x: 7, y: 1.8, w: 5.5, colW: [2.5, 3], border: { color: "DDDDDD", pt: 0.5 }, margin: [4, 8, 4, 8] }
);

// Speed improvement callouts
slide.addShape(pres.ShapeType.roundRect, { x: 0.8, y: 5.0, w: 3.5, h: 1.8, fill: { color: NAVY }, rectRadius: 0.15 });
slide.addText("개발 속도", { x: 0.8, y: 5.1, w: 3.5, fontSize: 12, color: GRAY, align: "center", fontFace: "Arial" });
slide.addText("33% 단축", { x: 0.8, y: 5.5, w: 3.5, fontSize: 32, color: WHITE, bold: true, align: "center", fontFace: "Arial Black" });
slide.addText("38주 → 25.5주", { x: 0.8, y: 6.2, w: 3.5, fontSize: 14, color: "2ECC71", align: "center", fontFace: "Arial" });

slide.addShape(pres.ShapeType.roundRect, { x: 5.0, y: 5.0, w: 3.5, h: 1.8, fill: { color: NAVY }, rectRadius: 0.15 });
slide.addText("QA 자동화", { x: 5.0, y: 5.1, w: 3.5, fontSize: 12, color: GRAY, align: "center", fontFace: "Arial" });
slide.addText("16개 에이전트", { x: 5.0, y: 5.5, w: 3.5, fontSize: 32, color: WHITE, bold: true, align: "center", fontFace: "Arial Black" });
slide.addText("84% 버그 발견율", { x: 5.0, y: 6.2, w: 3.5, fontSize: 14, color: "2ECC71", align: "center", fontFace: "Arial" });

slide.addShape(pres.ShapeType.roundRect, { x: 9.2, y: 5.0, w: 3.5, h: 1.8, fill: { color: NAVY }, rectRadius: 0.15 });
slide.addText("대표님 역할", { x: 9.2, y: 5.1, w: 3.5, fontSize: 12, color: GRAY, align: "center", fontFace: "Arial" });
slide.addText("의사결정자", { x: 9.2, y: 5.5, w: 3.5, fontSize: 32, color: WHITE, bold: true, align: "center", fontFace: "Arial Black" });
slide.addText("개발자 → 승인+방향", { x: 9.2, y: 6.2, w: 3.5, fontSize: 14, color: ICE, align: "center", fontFace: "Arial" });
